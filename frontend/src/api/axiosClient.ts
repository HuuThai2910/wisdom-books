...
axiosClient.interceptors.request.use((config) => {
    ...
    console.log(
        "[axiosClient] id_token cookie:",
        idToken ? "EXISTS (length: " + idToken.length + ")" : "NOT FOUND"
    );
...
axiosClient.interceptors.response.use(
    ...
    async (err) => {
        ...
        if (err.response?.status === 401) {
            ...
            if (
                (isAdminRoute || isStaffRoute || isWarehouseRoute) &&
                currentPath !== "/unauthorized"
            ) {
                console.log(
                    "[axiosClient] Redirecting to /unauthorized from:",
                    currentPath
                );
